...
import { PointnineZizumKey } from '../../../shared/types/pointnine.type';
import {
  pointnineShops,
  pointnineZizumNum,
} from '../../../shared/constants/shops/Pointnine';
import { pointnineThemes } from '../../../shared/constants/Themes';
...
const ThemeSelectForm = forwardRef((_props, ref) => {
  const [selectedShop, setSelectedShop] = useState<PointnineZizumKey | null>();
  ...
  useImperativeHandle(ref, () => {
    const cancel = async () => {
      try {
        const { data: cancel } = await axios.post(
          '/point-nine/core/res/rev.act.php',
          qs.stringify({
            name: '조준호',
    ...
    const reservation = async () => {
      ...
      try {
        try {
          const { data } = await axios.get('/point-nine/layout/res/home.php', {
            params: {
              rev_days: selectedDate,
              s_zizum: pointnineZizumNum[selectedShop],
              go: 'rev.make',
            },
          });
        ...
        const { data: revMake } = await axios.get(
          `/point-nine/layout/res/home.php`,
          {
            params: {
              go: 'rev.make.input',
              rev_days: selectedDate,
        ...
        const { data: result } = await axios.get(
          '/point-nine/core/res/rev.make.mutong.php',
          {
            params: {
              payment: 'D',
              num,
        ...
        const { data: message } = await axios.get(
          '/point-nine/core/res/rev.make.exe.php',
          {
            params: {
              num,
              ck_code,
      ...
      <Contents>
        <div>
          ...
          <Select
            value={selectedShop}
            style={{ width: 200 }}
            onChange={(selected: PointnineZizumKey) => {
              setSelectedShop(selected);
              setSelectedTheme('');
            }}
          >
            {Object.keys(pointnineShops).map((shopKey) => (
              <Select.Option key={shopKey} value={shopKey}>
                {pointnineShops[shopKey as PointnineZizumKey]}
              </Select.Option>
            ))}
          </Select>
        </div>
        <div>
          ...
          >
            {selectedShop &&
              pointnineThemes[selectedShop]?.map((theme) => (
